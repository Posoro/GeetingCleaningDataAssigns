################################################################################
# set working directory
#> getwd()
#[1] "C:/Users/Administrator/Documents"
#> setwd("C:/Users/Administrator/Documents/01Data Science/04GettingCdata/Week3/UCI HAR Dataset")
#> getwd()
# "C:/Users/Administrator/Documents/01Data Science/04GettingCdata/Week3/UCI HAR Dataset"
#> dir()
#[1] "activity_labels.txt" "features.txt"        "features_info.txt"   "README.txt"          "test"                "train"   

library(plyr)

# 1: Merge the training and test sets to create one data set
###############################################################################

#x_train <- read.table("train/X_train.txt")
x_trainset <- read.table("./train/X_train.txt")

y_trainset <- read.table("./train/y_train.txt")

subject_trainset <- read.table("./train/subject_train.txt")

x_testset <- read.table("./test/X_test.txt")
y_testset <- read.table("./test/y_test.txt")
subject_testset <- read.table("./test/subject_test.txt")

# create 'x' data set
x_data <- rbind(x_trainset, x_testset)

# create 'y' data set
y_data <- rbind(y_trainset, y_testset)

# create 'subject' data set
subject_data <- rbind(subject_trainset, subject_testset)

# 2 : Extract only the measurements on the mean and standard deviation for each measurement
###############################################################################

featureslist <- read.table("features.txt")

# get only columns with mean() or std() in their names
mean_and_std_features <- grep("-(mean|std)\\(\\)", features[, 2])

# subset the desired columns
x_data <- x_data[, mean_and_std_features]

# correct the column names
names(x_data) <- featureslist[mean_and_std_features, 2]

# 3: Use descriptive activity names to name the activities in the data set
###############################################################################

activities <- read.table("activity_labels.txt")

# update values with correct activity names
y_data[, 1] <- activities[y_data[, 1], 2]

# correct column name
names(y_data) <- "activity"

# 4: Appropriately label the data set with descriptive variable names
###############################################################################

# correct column name
names(subject_data) <- "subject"

# bind all the data in a single data set
all_data <- cbind(x_data, y_data, subject_data)

# 5: Create a second, independent tidy data set with the average of each variable
# for each activity and each subject
###############################################################################


averages_data  <- ddply(all_data, .(subject, activity), function(x) colMeans(x[, 1:66]))

write.table(averages_data, "tidy.txt", row.name=FALSE)
